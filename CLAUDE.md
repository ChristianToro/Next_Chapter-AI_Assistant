...
1. **`lib/tarkov.js`** queries the Tarkov Market REST API (`api.tarkov-market.app/api/v1[/pve]/item?q=`, Pro key in `TARKOV_MARKET_API_KEY`, sent as the `x-api-key` header) for up to 5 matches. tarkov.dev is deprecated; don't go back to it. Tarkov Market has no 24h low/high or 48h change, so `flea24hLow`/`flea24hHigh`/`change48h` are always `null`. `fleaAvg7d`, `change24h` and `bannedOnFlea` are returned but not yet used by the prompt. It **pre-formats every number into the exact display string** (`"₽ 412,300"`, `"+2.1%"`, `"2026-09-24 14:02 UTC"`) so the model only copies and never rounds. On API failure it returns `{error}` and does not throw.
...
- Tarkov Market's terms require crediting tarkov-market.com. The system prompt's `UPDATED` line, the few-shot answers, `public/terminal.js` help text, README and SPEC still say tarkov.dev and need updating.
